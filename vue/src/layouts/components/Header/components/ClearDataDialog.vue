<template>
	<el-dialog v-model="dialogVisible" :title="$t('header.clearData')" width="500px" draggable>
		<el-form :model="form" :rules="rules" ref="formRef" label-width="120px">
			<el-form-item :label="$t('login.password')" prop="password">
				<el-input v-model="form.password" :placeholder="$t('login.password')" type="password" show-password />
			</el-form-item>
			<el-form-item :label="$t('header.clearBusinessData')" prop="clearBillData">
				<el-switch disabled v-model="form.clearBillData"></el-switch>
			</el-form-item>
			<el-form-item :label="$t('header.clearBasicData')" prop="clearBillData">
				<el-switch v-model="form.clearBasicData"></el-switch>
			</el-form-item>
		</el-form>
		<template #footer>
...
<script setup lang="ts" name="ClearDataDialog">
import { ref } from "vue";
import { clearBillData } from "@/api/modules/login";
...
const onSubmit = () => {
	formRef.value!.validate(async (valid: boolean) => {
		if (valid) {
			ElMessageBox.confirm(useFormat18n("header.clearDataTips"), useFormat18n("common.tips"), {
