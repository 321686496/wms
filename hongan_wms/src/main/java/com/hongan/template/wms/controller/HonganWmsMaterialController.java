...
@Validated
@RestController
@RequestMapping("/admin/wms/material")
public class HonganWmsMaterialController extends BaseController {
    @Autowired
    private IHonganWmsMaterialService service;

    @ApiOperation("分页查询")
    @GetMapping
    public BaseResponse queryPage(Authentication auth, QueryMaterial query) throws BaseException {
        TokenUser user = (TokenUser) auth.getPrincipal();
//        IPage<HonganWmsMaterial> page = service.queryPage(query);
        IPage<HonganWmsMaterialVO> page = service.queryPageVO(query);
    ...
    @ApiOperation("查询列表")
    @GetMapping("/list")
    public BaseResponse queryList(Authentication auth, QueryMaterial query) throws BaseException {
        TokenUser user = (TokenUser) auth.getPrincipal();
//        List<HonganWmsMaterial> list = service.queryList(query);
        List<HonganWmsMaterialVO> list = service.queryListVO(query);
        return new BaseResponse().success(list);
    }

    @ApiOperation("新增")
    @PostMapping
    public BaseResponse create(Authentication auth, @RequestBody HonganWmsMaterial data) throws BaseException {
        TokenUser user = (TokenUser) auth.getPrincipal();
        service.create(data);
    ...
    @ApiOperation("修改")
    @PutMapping
    public BaseResponse update(Authentication auth, @RequestBody HonganWmsMaterial data) throws BaseException {
        TokenUser user = (TokenUser) auth.getPrincipal();
        service.update(data);
        return new BaseResponse().success();
    }
