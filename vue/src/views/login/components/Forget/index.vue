<template>
	<div class="register-block tabs-wrap">
		<el-tabs v-model="tabActive" stretch>
			<el-tab-pane :label="$t('login.reset') + $t('login.password')" name="forget">
...
import { resetPwdApi } from "@/api/modules/login";
...
const registerRules = reactive({
	...
	pwd: [
		{ required: true, message: useFormat18n(["common.pleaseInput", "login.newPassword"]), trigger: "blur" },
...
const onSubmit = () => {
	registerFormRef.value &&
		registerFormRef.value.validate(async valid => {
			...
			try {
				// 1.执行重置接口
				await resetPwdApi({ ...registerForm });
				ElMessageBox.confirm(`${useFormat18n(["login.reset", "common.success"])}`, useFormat18n("common.tips"), {
...
const onSmsClick = async () => {
	...
	registerFormRef.value &&
		registerFormRef.value.validateField(["phone", "captcha"], valid => {
			if (valid) {
				// 2. 触发获取短信验证码方法
				getSmsCode({ type: "forget", phone: registerForm.phone, imgCaptcha: registerForm.captcha });
			}
		});
};
