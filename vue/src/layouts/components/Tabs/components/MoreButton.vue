...
import { removeTableSearchCache } from "@/hooks/useTableCache";
...
const closeCurrentTab = () => {
	...
	setTimeout(() => {
		removeTableSearchCache(route.name as string);
	}, 1000);
...
// Close Other
const closeOtherTab = () => {
	let nameArr: string[] = tabStore.tabsMenuList.map(tab => tab.name);
	nameArr = nameArr.filter(item => item !== route.name);
	setTimeout(() => {
		removeTableSearchCache(nameArr);
	}, 1000);
...
// Close All
const closeAllTab = () => {
	const nameArr: string[] = tabStore.tabsMenuList.map(tab => tab.name);
	setTimeout(() => {
		removeTableSearchCache(nameArr);
	}, 1000);
