<template>
	<component
		...
		v-bind="{ ...handleSearchProps, ...placeholder, searchParam: _searchParam, clearable: true }"
		v-model="_searchParam[column.search?.key ?? handleProp(column.prop!)]"
		:data="column.search?.el === 'tree-select' ? columnEnum : []"
		:options="['cascader', 'select-v2', 'pl-search-select'].includes(column.search?.el!) ? columnEnum : []"
...
const placeholder = computed(() => {
	...
	const placeholder = search?.props?.placeholder ?? (label || (search?.el?.includes("input") ? "请输入" : "请选择"));
	return { placeholder };
});
