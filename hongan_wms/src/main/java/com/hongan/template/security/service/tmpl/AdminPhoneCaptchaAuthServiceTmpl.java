...
@Service("AdminPhoneSecurityService")
public class AdminPhoneCaptchaAuthServiceTmpl extends AbstractAuthService implements IAuthService {
    ...
    public void verify(HttpServletRequest request) throws BaseException, StepAuthException {
        ...
        if (!path.startsWith("dev")) {
            if (!phoneServiceCaptcha.verify(request, req.getName(), req.getCaptcha())) {
                throw new BaseException(BaseError.PhoneCaptchaError);
            }
        }
        //2.校验用户密码是否正确
        HonganAdmin admin = adminService.loginByPhone(req.getName());
        //3.更新最后登陆IP和时间
        String ipAddr = CusAccessObjectUtil.getIpAddress(request);
        adminService.uploadLoginInfo(admin.getId(), ipAddr);
        parameters = new SecurityParameters(admin.getName(), admin.getPassword(), request, PlSecurityConstConfig.AdminPhoneCaptchaLogin);
    }

    @Override
    public String getMatcher() {
        return PlSecurityConstConfig.AdminPhoneCaptchaLogin + ":post";
    }
